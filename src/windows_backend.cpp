// Windows memory backend. Wraps ReadProcessMemory / WriteProcessMemory
// on a process handle acquired with minimal rights. Detects partial
// reads/writes, returns the Win32 error code on failure, and reports
// process liveness by polling NtQuerySystemInformation-style state via
// GetExitCodeProcess (the lightweight, documented API).
//
// This translation unit is only compiled on Windows (see CMakeLists.txt
// setting GTLIBCPP_PLATFORM_WINDOWS and gating the include path).
...
private:
    ...
    static std::string query_image_sha256(std::uint32_t /*pid*/) {
        // Stub: the production build wires a maintained image-hash
        // library (e.g. OpenSSL EVP_Digest). Until then the policy
        // layer treats an empty hash as "no constraint", which is the
