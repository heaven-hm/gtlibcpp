// CheatTableParser — minimal XML reader, versioned CE 7.x subset.
//
// The parser is intentionally narrow:
//   * It accepts the elements that ship with the two checked-in
//     `.ct` fixtures (CheatTable / CheatEntries / CheatEntry / Variable
//     / Hotkey / AssemblyScript / comments) and nothing else.
//   * It rejects Auto Assembler, DLL loads, shell-exec, and unconstrained
//     byte writes — those are explicitly out of scope for issue #1.
//   * It preserves nesting, multiple hotkeys, string metadata, signed
//     values, and hex values.
//   * It returns per-entry diagnostics: every entry that is skipped is
//     listed in `unsupported_entries` with a reason.
//
// We use a small hand-rolled XML reader rather than pulling in pugixml
// or RapidXML, so the parser has no third-party surface and no
// transitive licensing obligations.
...
// ---------------------------------------------------------------------
// std::hash specialisation for EntryId (declared in parser.hpp).
// ---------------------------------------------------------------------

namespace {

// ---------------------------------------------------------------------
// Tiny XML reader (no DOM, just enough to walk CE 7.x tables).
// ---------------------------------------------------------------------
        ...
        while (!done()) {
            ...
            if (pos_ + 4 <= src_.size()
                ...
                if (src_[pos_ + 2] == '-' && src_[pos_ + 3] == '-') {
                    ...
                    ++pos_; // skip unknown declaration char
                }
            } else {
                break;
            }
...
std::string CheatTableParser::sanitize(const std::string& input) {
    ...
    for (char c : input) {
        ...
        if (uc == '\t' || uc == '\n' || uc == '\r') {
            ...
            // control characters; the legacy parser tolerated these and
            // the XML reader does not.
            out.push_back(' ');
        } else {
            out.push_back(c);
        }
