...
// Convert an error code to a stable, machine-readable string. The text is
// part of the public agent contract and must not change without a major
// version bump.
...
// Carries every piece of context the agent, the log, and the test need
// when a public operation does not succeed. Mirrors the "structured
// error" requirement from the issue: operation name, address, byte
// count, and underlying system error code (Win32 or errno) are always
...
// A discriminated union: either a T value (success) or an Error. We avoid
// std::expected to keep the header dependency surface narrow and to make
// the success/failure branch obvious in debugger and logs.
template <typename T>
class Result {
public:
    ...
    [[nodiscard]] const T& value() const& {
        if (!has_value_) {
            // The legacy API silently returned T{} on failure, which is
            // exactly the bug the issue is fixing. Tests that want to
            // assert on a value must check ok() first.
...
// Void specialization so functions that do not produce a value (rollback,
// cancel, approval) can still return structured errors.
template <>
class Result<void> {
public:
